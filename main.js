...
// 【重要】LINEのシステムを初期化する（一番上に書いてください）
// "YOUR_LIFF_ID_HERE" の部分を、ご自身の実際のLIFF IDに書き換えてください。
liff.init({ liffId: "2011383351-oeSaS7vS" })
.then(() => {
if (!liff.isLoggedIn()) {
liff.login(); // ログインしていなければログインさせる
}
})
.catch((err) => {
...
// ユーザーのLINEから公式LINEのトーク画面へテキストを自動送信する
liff.sendMessages([
{
type: 'text',
text: messageText
